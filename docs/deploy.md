...
| `BILLING_UNPARSEABLE_EVENT` | `BillingRoutes` | a signed webhook body did not match the shape `FreemiusWebhook.parse` reads | confirm the event type and the field names against the vendor SDK. The body never reaches the log line |
