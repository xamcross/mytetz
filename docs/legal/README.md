...
| The subscription row and an account deletion | Since issue #177: the deletion route answers `409 SUBSCRIPTION_ACTIVE` and changes nothing while the status is `ACTIVE` or `PAST_DUE`, the two statuses that can renew. For each other status the deletion proceeds and removes the subscription row too (`BillingService.deleteSubscriptionFor`). The route calls no Freemius cancel: the learner cancels first, through "Manage subscription". A late webhook for a deleted user changes nothing and makes no new row. | `backend/api/src/main/kotlin/com/mytetz/api/AuthRoutes.kt`, `POST /api/account/delete`; `backend/billing/src/main/kotlin/com/mytetz/billing/BillingService.kt`, `deleteSubscriptionFor` and `apply` |
