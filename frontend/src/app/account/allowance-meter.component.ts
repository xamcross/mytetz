      ...
      <div class="allowance-meter">
        @if (metered(account.status)) {
          ...
          >
            ...
            >
            <!--
              Issue #139. "tokens" makes the header's own short form below 768px too wide for a
              360px phone — a real run measured the wordmark and the "Account" link overlapping by
              about 3px, where the old, unit-less text left an 8px gap. This shorter form carries
              the same two numbers and the same unit, so a screen reader (which never reads this —
              it reads the full sentence below) and a sighted learner on a wide screen both keep
              the fuller text; only the narrow header swaps to this one — see the CSS below.
            -->
